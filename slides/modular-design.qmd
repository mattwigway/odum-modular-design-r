...
- Any place we refer to a variable containing a column name, use the "embracing operator" `{{ variable }}` to tell R to use the value contained by `variable` as the column name
 . . .
    - This is only true when the variable name would not usually be in quotes
...
- To install your custom package, you need to run `renv::install("path/to/package/directory")` from within your `renv` environment (i.e. your project with scripts, not your package)
